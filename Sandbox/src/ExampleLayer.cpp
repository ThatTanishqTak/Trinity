	...
	: Layer("ExampleLayer"), m_CameraController(1280.0f / 720.0f)
{
	...
	Trinity::Ref<Trinity::VertexBuffer> vertexBuffer = Trinity::VertexBuffer::Create(vertices, sizeof(vertices));
	Trinity::BufferLayout layout = {
		{ Trinity::ShaderDataType::Float3, "a_Position" },
		{ Trinity::ShaderDataType::Float4, "a_Color" }
	};
	vertexBuffer->SetLayout(layout);
	m_VertexArray->AddVertexBuffer(vertexBuffer);

	uint32_t indices[3] = { 0, 1, 2 };
	Trinity::Ref<Trinity::IndexBuffer> indexBuffer = Trinity::IndexBuffer::Create(indices, sizeof(indices) / sizeof(uint32_t));
	...
	Trinity::Ref<Trinity::VertexBuffer> squareVB = Trinity::VertexBuffer::Create(squareVertices, sizeof(squareVertices));
	squareVB->SetLayout({
		{ Trinity::ShaderDataType::Float3, "a_Position" },
		{ Trinity::ShaderDataType::Float2, "a_TexCoord" }
		});
	m_SquareVA->AddVertexBuffer(squareVB);

	uint32_t squareIndices[6] = { 0, 1, 2, 2, 3, 0 };
	Trinity::Ref<Trinity::IndexBuffer> squareIB = Trinity::IndexBuffer::Create(squareIndices, sizeof(squareIndices) / sizeof(uint32_t));
...
void ExampleLayer::OnUpdate(Trinity::Timestep ts)
{
	// Update
...
void ExampleLayer::OnEvent(Trinity::Event& e)
{
