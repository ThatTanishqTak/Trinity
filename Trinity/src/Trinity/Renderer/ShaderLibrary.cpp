...
namespace Trinity
{
    ...
    std::shared_ptr<Shader> ShaderLibrary::Load(const std::string& identifier, VulkanContext* context, const std::filesystem::path& path, VkShaderStageFlagBits stage)
    {
    ...
    std::shared_ptr<Shader> ShaderLibrary::CreateFallback(VulkanContext* context)
    {
        static const uint32_t s_FallbackVert[] = 
        {
            ...
            0x0000000d, 0x00000000, 0x00040005, 0x00000012, 0x736f5061, 0x00000000, 0x00030047, 0x0000000b,
        ...
        static const uint32_t s_FallbackFrag[] = 
        {
        ...
        auto shader = std::make_shared<Shader>(context);

        auto temp = std::filesystem::temp_directory_path();
        auto vertPath = temp / "trinity_fallback.vert.spv";
        auto fragPath = temp / "trinity_fallback.frag.spv";

        {
            std::ofstream out(vertPath, std::ios::binary);
            out.write(reinterpret_cast<const char*>(s_FallbackVert), sizeof(s_FallbackVert));
        }

        {
            std::ofstream out(fragPath, std::ios::binary);
            out.write(reinterpret_cast<const char*>(s_FallbackFrag), sizeof(s_FallbackFrag));
        }

        shader->Load(vertPath, VK_SHADER_STAGE_VERTEX_BIT);
        shader->Load(fragPath, VK_SHADER_STAGE_FRAGMENT_BIT);
