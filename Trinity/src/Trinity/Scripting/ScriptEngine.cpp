...
namespace Trinity {
	...
	static std::unordered_map<std::string, ScriptFieldType> s_ScriptFieldTypeMap =
	{
		...
		{ "Trinity.Vector2", ScriptFieldType::Vector2 },
		{ "Trinity.Vector3", ScriptFieldType::Vector3 },
		{ "Trinity.Vector4", ScriptFieldType::Vector4 },

		{ "Trinity.Entity", ScriptFieldType::Entity },
	};
	...
	namespace Utils {
		...
		ScriptFieldType MonoTypeToScriptFieldType(MonoType* monoType)
		{
			...
			{
				TR_CORE_ERROR("Unknown type: {}", typeName);
	...
	void ScriptEngine::Init()
	{
		...
		InitMono();
		LoadAssembly("Resources/Scripts/Trinity-ScriptCore.dll");
	...
	void ScriptEngine::OnUpdateEntity(Entity entity, Timestep timestep)
	{
		...
		TR_CORE_ASSERT(s_Data->EntityInstances.find(entityUUID) != s_Data->EntityInstances.end());
	...
	void ScriptEngine::LoadAssemblyClasses()
	{
		...
		for (int32_t i = 0; i < numTypes; i++)
		{
			...
			TR_CORE_WARN("{} has {} fields:", className, fieldCount);
			...
			while (MonoClassField* field = mono_class_get_fields(monoClass, &iterator))
			{
				...
				if (flags & FIELD_ATTRIBUTE_PUBLIC)
				{
					...
					TR_CORE_WARN("  {} ({})", fieldName, Utils::ScriptFieldTypeToString(fieldType));
	...
	{
		TR_CORE_ASSERT(s_Data->EntityInstances.find(uuid) != s_Data->EntityInstances.end());
